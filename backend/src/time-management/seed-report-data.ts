...
async function seedReportData() {
  ...
  if (
    deptChoice &&
    parseInt(deptChoice) > 0 &&
    parseInt(deptChoice) <= departments.length
  ) {
  ...
  const validTypes = employeeTypes.filter(
    (t: any) => t !== null && t !== undefined && t !== '',
  );
  ...
  uniqueTypes.forEach((type: string, index: number) => {
    const count = employeeTypes.includes(type)
      ? '(exists in DB)'
      : '(no employees yet)';
  ...
  if (
    typeChoice &&
    parseInt(typeChoice) > 0 &&
    parseInt(typeChoice) <= uniqueTypes.length
  ) {
  ...
  const exceptionCountInput = await prompt(
    rl,
    '👉 How many exceptions to generate? (default: 20): ',
  );
  ...
  const daysInput = await prompt(
    rl,
    '👉 How many days of attendance data? (default: 30): ',
  );
  ...
  if (selectedDepartment) {
    employeeQuery.primaryDepartmentId = new Types.ObjectId(
  ...
  if (employees.length === 0) {
    ...
    console.log(
      '   Try selecting different filters or ensure employees exist in the database.',
    );
    await app.close();
    return;
  }
    ...
    for (const date of recordDays) {
      ...
      if (isLate) {
        lateMinutes = Math.round(
          (clockInTime.getTime() - expectedStart.getTime()) / 60000,
        );
      }
  ...
  const shuffledRecords = attendanceRecordsForExceptions.sort(
    () => Math.random() - 0.5,
  );
  const recordsForExceptions = shuffledRecords.slice(
    0,
    Math.min(exceptionCount, shuffledRecords.length),
  );
  ...
  for (const record of recordsForExceptions) {
    ...
    const exceptionStatus =
      exceptionStatuses[Math.floor(Math.random() * exceptionStatuses.length)];
  ...
  const overtimeRecords = await AttendanceRecord.countDocuments({
    totalWorkMinutes: { $gt: 480 },
  });
  const missedPunchRecords = await AttendanceRecord.countDocuments({
    hasMissedPunch: true,
  });
