...
export class PerformanceService {
  ...
  async bulkAssignAppraisals(
    dto: BulkAssignAppraisalsDto,
    ...
    const assignments = dto.employees.map((emp) => ({
      ...
      positionId: emp.positionId
        ? new Types.ObjectId(emp.positionId)
        : undefined,
      dueDate: dto.dueDate ? new Date(dto.dueDate) : undefined,
    }));
  ...
  async finalizeAppraisal(
    ...
    if (!record) {
      throw new NotFoundException(
        `No record found for assignment ${assignmentId}`,
      );
    }
    ...
    await this.employeeModel
      .findByIdAndUpdate(assignment.employeeProfileId, {
  ...
  async bulkPublishAppraisals(
    ...
    for (const assignmentId of dto.assignmentIds) {
      try {
        const result = await this.finalizeAppraisal(
          assignmentId,
          employeeIdWhoPublished,
        );
  ...
  async getMyPublishedAppraisals(
    employeeId: string,
  ): Promise<AppraisalAssignment[]> {
    return this.assignmentModel
      .find({
  ...
  async getAppraisalProgress(): Promise<any> {
    ...
    const notStarted = await this.assignmentModel
      .countDocuments({ status: AppraisalAssignmentStatus.NOT_STARTED })
      .exec();
    const inProgress = await this.assignmentModel
      .countDocuments({ status: AppraisalAssignmentStatus.IN_PROGRESS })
      .exec();
    const submitted = await this.assignmentModel
      .countDocuments({ status: AppraisalAssignmentStatus.SUBMITTED })
      .exec();
    const published = await this.assignmentModel
      .countDocuments({ status: AppraisalAssignmentStatus.PUBLISHED })
      .exec();
    const acknowledged = await this.assignmentModel
      .countDocuments({ status: AppraisalAssignmentStatus.ACKNOWLEDGED })
      .exec();
    ...
    return {
      ...
      completionRate:
        total > 0
          ? (((submitted + published + acknowledged) / total) * 100).toFixed(2)
          : 0,
    };
  }
