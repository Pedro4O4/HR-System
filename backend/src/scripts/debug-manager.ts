...
async function bootstrap() {
  ...
  const ahmed = await employeeModel
    .findOne({
      $or: [
        {
          firstName: { $regex: new RegExp('^Ahmed$', 'i') },
          lastName: { $regex: new RegExp('^Mohsen$', 'i') },
        },
  ...
  if (!ahmed) {
    ...
    const allEmps = await employeeModel
      .find({})
      .select('firstName lastName fullName')
      .exec();
    console.log(
      'Available Employees:',
      allEmps.map((e) => `${e.firstName} ${e.lastName}`),
    );
    return;
  }

  console.log(
    `Found Manager: ${ahmed.firstName} ${ahmed.lastName}, ID: ${ahmed._id}`,
  );

  // 2. Find Assignments for this Manager
  const assignments = await assignmentModel
    .find({ managerProfileId: ahmed._id })
    .exec();
  console.log(
    `Found ${assignments.length} assignments for Manager ID ${ahmed._id}`,
  );
  ...
  if (assignments.length === 0) {
    ...
    allAssignments.forEach((a) => {
      ...
      console.log(
        `Assignment ${a._id}: Assigned to Manager ${mgr?.firstName} ${mgr?.lastName} (ID: ${mgr?._id})`,
      );
    });
  } else {
