...
export class OrganizationStructureService {
  constructor(
    ...
    @InjectModel('StructureChangeRequest')
    private structureChangeRequestModel: Model<any>,
  ...
  async createChangeRequest(createChangeRequestDto: any): Promise<any> {
    try {
      ...
      const count = await this.structureChangeRequestModel
        .countDocuments()
        .exec();
      ...
      const employee = await this.employeeProfileModel
        .findById(createChangeRequestDto.requestedByEmployeeId)
        .exec();
      if (!employee) {
        throw new NotFoundException(
          `Employee with ID ${createChangeRequestDto.requestedByEmployeeId} not found`,
        );
      }
      ...
      const requestData = {
        requestNumber,
        requestedByEmployeeId: new Types.ObjectId(
          createChangeRequestDto.requestedByEmployeeId,
        ),
        ...
        submittedByEmployeeId: new Types.ObjectId(
          createChangeRequestDto.requestedByEmployeeId,
        ),
      ...
      console.log(
        `[CREATE CHANGE REQUEST] Successfully created request ${requestNumber}`,
      );
  ...
  async findAllChangeRequests(
    ...
    const total = await this.structureChangeRequestModel
  ...
  async approveChangeRequest(id: string): Promise<any> {
    try {
      const request = await this.structureChangeRequestModel
        .findById(id)
        .exec();
      if (!request) {
        throw new NotFoundException(`Change Request with ID ${id} not found`);
      ...
      try {
        ...
        console.warn(
          `[APPROVE] Could not parse details for request ${id}:`,
          e.message,
        );
      }
      ...
      if (request.requestType === StructureRequestType.NEW_POSITION) {
        ...
        const code =
          details.code || `POS-${String(count + 1).padStart(6, '0')}`;

        let departmentId =
          details.departmentId || request.targetDepartmentId?.toString();
        ...
        if (
          ...
          throw new BadRequestException(
            'Department is required for a new position. Please specify a department.',
          );
        }
        ...
        console.log(
          `[APPROVE] Created NEW_POSITION: ${code} in dept ${departmentId}`,
        );
        ...
        const code =
          details.code || `DEPT-${String(count + 1).padStart(4, '0')}`;
      ...
      console.log(
        `[APPROVE] Successfully updated request ${id} to APPROVED and executed changes`,
      );
  ...
  async rejectChangeRequest(id: string): Promise<any> {
    try {
      const updated = await this.structureChangeRequestModel
        .findByIdAndUpdate(
          id,
  ...
  async getMyTeam(managerId: string): Promise<any[]> {
    ...
    const myAssignments = await this.positionAssignmentModel
      .find({
    ...
    console.log(
      `[MyTeam] Manager ${managerId} position IDs:`,
      myPositionIdStrings,
    );
    ...
    console.log(
      `[MyTeam] Found ${directReportPositions.length} direct report positions.`,
    );
    directReportPositions.forEach((p) =>
      console.log(
        ` - Position: ${p.title} (${p._id}) reports to ${p.reportsToPositionId}`,
      ),
    );
    ...
    const directReportPositionIdStrings = directReportPositionIds.map((id) =>
      id.toString(),
    );
    ...
    const teamAssignments = await this.positionAssignmentModel
      ...
      .populate(
        'employeeProfileId',
        'firstName lastName workEmail profilePictureUrl',
      )
    ...
    console.log(
      `[MyTeam] Final team size for ${managerId}: ${finalTeam.length}`,
    );
    finalTeam.forEach((m) =>
      console.log(` - Member: ${m.name} (${m.position})`),
    );
  ...
  async removeDepartment(id: string): Promise<Department> {
    ...
    const positionsInDept = await this.positionModel
      .countDocuments({ departmentId: id })
      .exec();
  ...
  async createPosition(
    ...
    const department = await this.departmentModel
      .findById(createPositionDto.departmentId)
      .exec();
    if (!department) {
      throw new NotFoundException(
  ...
  async assignPosition(
    ...
    const { employeeProfileId, positionId, startDate, reason, notes } =
      assignPositionDto;
    ...
    if (!employee) {
      throw new NotFoundException(
        `Employee with ID ${employeeProfileId} not found`,
      );
    }
