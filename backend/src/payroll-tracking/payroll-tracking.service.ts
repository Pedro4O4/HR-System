...
export class PayrollTrackingService {
    ...
    try {
      ...
      if (filter.employeeId) {
        if (Types.ObjectId.isValid(filter.employeeId)) {
          ...
          console.warn(
            `Invalid employeeId format in filter: ${filter.employeeId}`,
          );
          return [];
        }
      }
  ...
  async updateClaimStatus(
    ...
    if (role === 'Payroll Specialist')
      claim.payrollSpecialistId = new Types.ObjectId(userId);
    if (role === 'Payroll Manager')
      claim.payrollManagerId = new Types.ObjectId(userId);
    if (role === 'Finance Staff')
      claim.financeStaffId = new Types.ObjectId(userId);
  ...
  async updateDisputeStatus(
    ...
    if (role === 'Payroll Specialist')
      dispute.payrollSpecialistId = new Types.ObjectId(userId);
    if (role === 'Payroll Manager')
      dispute.payrollManagerId = new Types.ObjectId(userId);
    if (role === 'Finance Staff')
      dispute.financeStaffId = new Types.ObjectId(userId);
